import os
import random
import re
import sys

DAMPING = 0.85
SAMPLES = 10000


def main():
    if len(sys.argv) != 2:
        sys.exit("Usage: python pagerank.py corpus")
    corpus = crawl(sys.argv[1])
    ranks = sample_pagerank(corpus, DAMPING, SAMPLES)
    print(f"PageRank Results from Sampling (n = {SAMPLES})")
    for page in sorted(ranks):
        print(f"  {page}: {ranks[page]:.4f}")
    ranks = iterate_pagerank(corpus, DAMPING)
    print(f"PageRank Results from Iteration")
    for page in sorted(ranks):
        print(f"  {page}: {ranks[page]:.4f}")


def crawl(directory):
    """
    Parse a directory of HTML pages and check for links to other pages.
    Return a dictionary where each key is a page, and values are
    a list of all other pages in the corpus that are linked to by the page.
    """
    pages = dict()

    # Extract all links from HTML files
    for filename in os.listdir(directory):
        if not filename.endswith(".html"):
            continue
        with open(os.path.join(directory, filename)) as f:
            contents = f.read()
            links = re.findall(r"<a\s+(?:[^>]*?)href=\"([^\"]*)\"", contents)
            pages[filename] = set(links) - {filename}

    # Only include links to other pages in the corpus
    for filename in pages:
        pages[filename] = set(
            link for link in pages[filename]
            if link in pages
        )

    return pages


def transition_model(corpus, page, damping_factor):
    """
    Return a probability distribution over which page to visit next,
    given a current page.

    With probability `damping_factor`, choose a link at random
    linked to by `page`. With probability `1 - damping_factor`, choose
    a link at random chosen from all pages in the corpus.
    """
    probability_distribution = {}
    dictionary_length = len(corpus.keys())
    pages_length = len(corpus[page])
    if len(corpus[page]) < 1:
        for key in corpus.keys():
            probability_distribution[key] = 1 / dictionary_length
    else:
        random_factor = (1 - damping_factor) / dictionary_length
        even_factor = damping_factor / pages_length
        for key in corpus.keys():
            if key not in corpus[page]:
                probability_distribution[key] = random_factor
            else:
                probability_distribution[key] = even_factor + random_factor
    return probability_distribution


def sample_pagerank(corpus, damping_factor, n):
    """
    Return PageRank values for each page by sampling `n` pages
    according to transition model, starting with a page at random.

    Return a dictionary where keys are page names, and values are
    their estimated PageRank value (a value between 0 and 1). All
    PageRank values should sum to 1.
    """
    samples_dictionary = corpus.copy()
    for i in samples_dictionary:
        samples_dictionary[i] = 0
    sample = None
    for i in range(n):
        if sample:
            distribution = transition_model(corpus, sample, damping_factor)
            distribution_list = list(distribution.keys())
            distribution_weights = [distribution[i] for i in distribution]
            sample = random.choices(distribution_list, distribution_weights, k=1)[0]
        else:
            sample = random.choice(list(corpus.keys()))
        samples_dictionary[sample] += 1
    for item in samples_dictionary:
        samples_dictionary[item] /= n
    return samples_dictionary

def iterate_pagerank(corpus, damping_factor):
    """
    Return PageRank values for each page by iteratively updating
    PageRank values until convergence.

    Return a dictionary where keys are page names, and values are
    their estimated PageRank value (a value between 0 and 1). All
    PageRank values should sum to 1.
    """
    pages_number = len(corpus)
    old_dictionary = {}
    new_dictionary = {}
    for page in corpus:
        old_dictionary[page] = 1 / pages_number
    while True:
        for page in corpus:
            temp = 0
            for linking_page in corpus:
                if page in corpus[linking_page]:
                    temp += (old_dictionary[linking_page] / len(corpus[linking_page]))
                if len(corpus[linking_page]) == 0:
                    temp += (old_dictionary[linking_page]) / len(corpus)
            temp *= damping_factor
            temp += (1 - damping_factor) / pages_number
            new_dictionary[page] = temp
        difference = max([abs(new_dictionary[x] - old_dictionary[x]) for x in old_dictionary])
        if difference < 0.001:
            break
        else:
            old_dictionary = new_dictionary.copy()
    return old_dictionary


if __name__ == "__main__":
    main()
